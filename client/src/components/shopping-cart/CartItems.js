...
const CartItems = ({ product, quantity, handleAddOnClick, handleRemoveItemOnClick, handleRemoveProductOnClick }) => {
    return (
        <div className='cart-items-container'>
            <i className="fas fa-times-circle i-remove-color" onClick={handleRemoveProductOnClick}></i>
            <figure>
                <img src={product.image} alt={product.name} />
            </figure>
            <h3>{product.name}</h3>
            <p>$ {product.discount && product.discount > 0
                ? Number(
